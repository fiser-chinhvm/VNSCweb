...
public class WCS {
    ...
    public Response getFullImage(@PathParam("name") String name) throws IOException {
        ...
        if(name.startsWith("LC")) {
            input = ImageIO.createImageInputStream(new File(path.getValue("Path") + "/geotiff/" + name + "/" + name + ".jpg"));
        }
        if(name.startsWith("M")){
            name = name.substring(0, name.length() - 4);
            input = ImageIO.createImageInputStream(new File(path.getValue("Path") + "/modis/" + name + "/BROWSE." + name + ".1.jpg"));
        }

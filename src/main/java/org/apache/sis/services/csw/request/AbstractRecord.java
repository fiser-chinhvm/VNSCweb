 ...
 * Dublin Core elements. The mapping from ISO 19115 metadata to
 * {@code SummaryRecord} is defined in:
 *
 * <blockquote>
 * OpenGIS Catalog Service Specification — ISO metadata application profile (OGC
 * 07-045), table 6 at page 41.
 * </blockquote>
 *
 ...
 * @module
 */
@XmlRootElement(name = "AbstractRecord", namespace = Namespaces.CSW)
@XmlType(name = "AbstractRecordType", propOrder = {
    "creator",
    "contributor",
    "publisher",
    "identifier",
...
public class AbstractRecord extends CswConfigure {
    ...
    /**
     * An unique reference to the record within the catalogue. This is mapped to
     * the {@code metadata/metadataIdentifier} property of ISO 19115.
     */
    @XmlElement(namespace = DUBLIN_CORE)
    String identifier;
     ...
     * general categories, genres or aggregation levels of content. This is
     * mapped to the {@code metadata/metadataScope/resourceScope} property of
     * ISO 19115. If more than one scope is provided, only the first one is
     * retained.
     */
    @XmlElement(namespace = DUBLIN_CORE)
    String type;
     ...
     * This is a DCMI metadata term
     * {@code <http://dublincore.org/documents/dcmi-terms/>}. mapped to the
     * {@code metadata/dateInfo} property of ISO 19115, taking in account only
     * dates associated to {@link DateType#CREATION} or
     * {@link DateType#LAST_UPDATE}. If more than one date exist for those
     * types, then only the latest date is retained.
     */
     ...
     * may include the media-type or dimensions of the resource. Format may be
     * used to determine the software, hardware or other equipment needed to
     ...
     * If the metadata cites more than one format, then the titles of all
     * formats (omitting duplicated values) are included in this field,
     * separated by new-line characters.</p>
     */
    @XmlElement(namespace = DUBLIN_CORE)
    String format;
     ...
     */
    public AbstractRecord(final Metadata object, String version, String service) {
        super();
        this.version = version;
        this.service = service;
        this.creator = first(first(first(object.getIdentificationInfo()).getCitation().getCitedResponsibleParties()).getParties()).getName().toString();
        this.contributor = first(first(first(object.getIdentificationInfo()).getCitation().getCitedResponsibleParties()).getParties()).getName().toString();
        ...
        this.modified = first(first(object.getIdentificationInfo()).getCitation().getDates()).getDate();
        ...
        final ImageDescription  contentInfo = (ImageDescription) first(object.getContentInfo()) ;
        Double a = contentInfo.getCloudCoverPercentage() ;
        if( a == null){
            a = -1.0;
        }
        this.cloudcover = a;
     ...
     * system default.
     */
    public AbstractRecord(final Metadata metadata, final Locale locale) {
        /*
    ...
    /**
     * Return a Cloud cover for identifying the value cloud cover for image.
     */
     public Double getCloudcover() {
        return cloudcover;
    }
     /**
     * Set a Cloud cover for identifying the value cloud cover for image.
     */
    public void setCloudcover(Double cloudcover) {
        this.cloudcover = cloudcover;
    }
    /**
     * Return the topic of the content of the resource.
     */
    ...
    /**
     * Formats the elements in the given set as a list separated by the given
     * string. This method avoid to create a temporary buffer when the set
